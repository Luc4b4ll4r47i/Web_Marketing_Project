...
# Open rate
table(df_6_camp_event_clean_final$OPENED)['TRUE']/dim(df_6_camp_event_clean_final)[1]*100

# per cli
open_rate_per_cli<- df_6_camp_event_clean_final  %>%
  group_by(ID_CLI) %>%
  summarize(TOT_EVENTs=n_distinct(ID_EVENT_S), TOT_OPENED=table(OPENED)['TRUE'])

open_rate_per_cli$OPEN_RATE<-round(((open_rate_per_cli$TOT_OPENED/open_rate_per_cli$TOT_EVENTs)*100), digits = 2)
open_rate_per_cli[is.na(open_rate_per_cli)] <- 0

# Click-Through Rate on OPENED SENT EVENT
table(df_6_camp_event_clean_final$CLICKED)['TRUE']/dim(df_6_camp_event_clean_final[df_6_camp_event_clean_final$OPENED=='TRUE', ])[1]*100
table(df_6_camp_event_clean_final$CLICKED)['TRUE']/dim(df_6_camp_event_clean_final)[1]*100

# per cli
ctr_per_cli<- df_6_camp_event_clean_final  %>%
  filter(OPENED=='TRUE') %>%
  group_by(ID_CLI) %>%
  summarize(TOT_EVENTs=n_distinct(ID_EVENT_S), TOT_CLICKED=table(CLICKED)['TRUE'])

ctr_per_cli$CTR<-round(((ctr_per_cli$TOT_CLICKED/ctr_per_cli$TOT_EVENTs)*100), digits = 2)
ctr_per_cli[is.na(ctr_per_cli)] <- 0
...
tot_daystoopen <- n_distinct(df6_dist_daystoopen$AVG_DAYS_TO_OPEN)
...
plot_df6_dist_numopens <- (
  ggplot(data=df_6_dist_numopens
         , aes(x=NUM_OPENs, y=TOT_CLI)) +
    geom_bar(stat="identity"
             , fill="steelblue")+
...
# - NUM_CLICKs
df_6_dist_numclicks <- df_6_camp_event_clean_final %>%
  mutate(NUM_CLICKs = ifelse(NUM_CLICKs > 3, "More than 3", NUM_CLICKs)) %>%
  group_by(NUM_CLICKs) %>%
  summarize(TOT_CLI = n_distinct(ID_CLI)) %>%
  mutate(PERCENT = TOT_CLI/sum(TOT_CLI)*100) %>%
  arrange(desc(PERCENT)) %>%
...
plot_df6_dist_numclicks <- (
  ggplot(data=df_6_dist_numclicks
         , aes(x=NUM_CLICKs, y=TOT_CLI)) +
    geom_bar(stat="identity"
             , fill="steelblue")+
    geom_text(aes(label=round(PERCENT, digits = 2)), position=position_dodge(width=0.9), vjust=-0.15)
)
